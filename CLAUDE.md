...
Schema **18**, service worker **planner-v28**.
...
cache**, so everything a cache serves came from one deploy; refreshing per file
once let a page run one version's JS against another's CSS.
...
(`supabase/upgrade.sql`, idempotent, named by `describeSyncError()` on a
rejected push). That is why habits, links, the wishlist and the chart's bands
ride inside `meta`, and why `applyRow` takes `data.links`, `data.wishlist` and
`data.sprints` only when the key is **present**: an older device sends none,
...
version did is in `migrate()` and `js/changelog.js`; 18 added `sprints`,
`freewrite`, and the Sunday week.
...
`reorderId`). **A category row's caret is appended after the label, never
prepended** — anything before the glyph breaks the sidebar's one alignment.
...
written**, as `notes[date].journal[areaId]`, so a year of writing syncs as small
rows rather than one row resent whole; `emptyNote()` counts it, or an entry on
an otherwise blank day never leaves the device. The **freewrite** is the
opposite shape: one string on the area, no dates, no history.
...
as `reserve`. **A focus or a sprint is a stretch of weeks in one area's lane**,
swept out horizontally the way a block is swept out on the calendar — same
object, `kind` deciding whether deliverables are asked for, drawn *above* the
work with item lanes offset by their height. SMART lives in the wording of a
deliverable, not in five labelled fields.
...
- **Never autosize a textarea with no width.** One measured at zero wraps every
  ...
  watches width only — reacting to its own height change would never settle.
- `body.rail-hidden` makes `#app` a **single-column** grid; `0 1fr` would leave
  main in the zero-width column and render a blank page. `.pad` is
  `margin-inline: auto`, or right-anchored controls stop at the 1180px cap.
- Quick add checks `parseLinkAdd()` **first** — a URL at the front is a bookmark
  — and `parseRange()` before `parseWhen()`, or half of "12-7" becomes a due
  time and the rest is stranded in the title.
- A link's title comes from its URL and nothing else — a page's own `<title>` is
  ...
  when clicked; a Drive URL holds only an id, so `linkTitleFromUrl` names the
  *kind*: Google Doc, Drive file, Drive folder.
- Capture's **Enter must stay the shortest path out** — an unfiled note, never a
  ...
  either strands every capture. `habitStreak()` forgives an untouched today but
  not a missed yesterday.
...
Serve the repo, open `/tests/`. No runner, no dependencies, 486 checks, and
...
`storeWith(raw)`; but a fresh instance has its own `state`, so a suite that
pokes state *and* calls into `gcal.js`/`cloud.js` must use `sharedStoreWith()`
— once per page, since `import()` caches. To add a field to a task: the literal
in `upsertItem()`, a fallback in `migrate()` so saved data still loads, a row in
`js/editor.js`. Sync ships whatever shape the object has.
...
| dragging a time range on a grid · sweeping a band on the chart | `js/timegrid.js` · `views/semester.js`, `js/sprint.js` |
| toasts, modals, peek, drag, reorder · the task panel | `js/ui.js` · `js/editor.js` |
| patch notes and the version · Google / Supabase sync | `js/changelog.js` · `js/gcal.js`, `js/cloud.js`, `supabase/*.sql` |
